...
// 命令行参数的默认值（-1 表示使用默认值 0）
int g_defaultMin = -1, g_defaultSec = -1;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        // === 关键：如果命令行参数提供了默认值，就填入 ===
        if (g_defaultMin >= 0) {
            wchar_t buf[10];
            swprintf(buf, 10, L"%d", g_defaultMin);
            SetWindowTextW(g_hEditMin, buf);
            swprintf(buf, 10, L"%d", g_defaultSec);
