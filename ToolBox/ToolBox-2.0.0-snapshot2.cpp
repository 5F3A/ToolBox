...
void InitTexts() {
    ...
    // -----
    texts[TXT_VER_INFO + LANG_ZH_CN] = "Tool-Box 2.0.0-snapshot2";
    ...
    texts[TXT_UPDLOG_CONTENT + LANG_ZH_CN] =
        ...
        "2.0.0-sp1: Nothings\n"
        "2026-08-30\n"
        "2.0.0-snapshot2: 增加 TIME, LOOPTIME, DATE, ALARM 指令，ALARM 为独立 GUI 程序";
    ...
    // -----
    texts[TXT_VER_INFO + LANG_EN_US] = "Tool-Box 2.0.0-snapshot2";
    ...
    texts[TXT_UPDLOG_CONTENT + LANG_EN_US] =
        ...
        "2.0.0-sp1: Nothings\n"
        "2026-08-30\n"
        "2.0.0-snapshot2: Added TIME, LOOPTIME, DATE, ALARM commands, ALARM is standalone GUI";
...
static void ToolBox(int argc, char** argv, bool debuginput = false) {
    ...
    while (true) {
        ...
        // ----- 新增命令 -----
            ...
            cout << GetText(TXT_ALARM_LAUNCH) << endl;
            // 启动外部 AlarmClock.exe（假设在同一目录）
            system("start AlarmClock.exe");
        }
        // ----- 结束新增 -----
        else if (command == "") {
            // 空指令无操作
