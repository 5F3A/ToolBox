...
void InitTexts() {
    ...
    texts[TXT_VER_INFO + LANG_ZH_CN] = "Tool-Box 2.0.0-snapshot5";
...
2026-08-31
2.0.0-snapshot3: ALARM 自动查找 AlarmClock-*.exe，集成 JIYUKILL
2026-09-10
2.0.0-snapshot5: 增加 TIMER 指令，自动查找 Timer-*.exe
)";
...
如果你想要在此处看到你的名字，请至少协助 1 个版本的开发（通过 https://space.bilibili.com/3546939977107567 联系，并且关注我）
    ...
    texts[TXT_TIMER_NOT_FOUND + LANG_ZH_CN] = "错误：找不到 Timer-*.exe 文件，请确保计时器程序在工具箱目录下。";
    ...
    texts[TXT_VER_INFO + LANG_EN_US] = "Tool-Box 2.0.0-snapshot5";
...
2026-08-31
2.0.0-snapshot3: ALARM auto-finds AlarmClock-*.exe, integrated JIYUKILL
2026-09-10
2.0.0-snapshot5: Added TIMER command, auto-finds Timer-*.exe
)";
...
If you want your name here, please help develop at least 1 version (contact via Bilibili)
    ...
    texts[TXT_TIMER_NOT_FOUND + LANG_EN_US] = "Error: Timer-*.exe not found. Please place the timer program in the toolbox directory.";
}
...
static void ToolBox(int argc, char** argv, bool debuginput = false) {
    ...
    while (true) {
        ...
        if (command == "HELP") {
        ...
        else if (command == "DEBUG") debug = true;
            ...
            if (found) {
                ...
                cout << "（例如：AlarmClock-1.1.1.exe）" << endl;
            }
        }
